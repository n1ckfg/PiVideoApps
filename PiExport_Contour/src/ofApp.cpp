...
void ofApp::update() {
    ...
    if (counter < inputUrls.size()) {
        ...
        fbo1.allocate(gray.getWidth(), gray.getHeight(), GL_RGBA);
...
void ofApp::draw() {
    ...
    fbo1.draw(0, 0, width/2, height);
    gray.draw(width/2, 0, width/2, height);
    
    fbo2.end();
    
    fbo2.readToPixels(outputPixels);
    ofSaveImage(outputPixels, ofFilePath::join(outputUrl, ofToString(counter)) + "." + outputFileType);
